...
class AdvancedBuyAndHoldStrategy:
    ...
    def start_trading_sim(self):
        ...
        for d in range(len(self.signal)):
            ...
            close_price = self.q * self.close[d]

            # if self.dns.Signal[d] == 1:
            # if self.dns.Signal[d] == 1 and len(self.open_orders) < self.max_open_orders_permitted and self.cashBalance - close_price > 500:
            if self.signal[d] == 1 and self.cashBalance - close_price > 0:

            # if self.dns.Signal[d] == 1 and len(self.open_orders) < self.max_open_orders_permitted:
                ...
                self.cashBalance = self.cashBalance - close_price

                post_trade_open_orders = [x for x in self.open_orders]
                post_trade_cash_balance = pre_trade_cash_balance - close_price
                post_trade_total_assets = post_trade_cash_balance + sum(post_trade_open_orders)

                self.print_display_buy_event_info(close_price, pre_trade_cash_balance,
                                                  pre_trade_total_assets, pre_trade_open_orders,
                                                  post_trade_cash_balance, post_trade_total_assets,
                                                  post_trade_open_orders)
