...
class Order:
    ...
    def __init__(self, open_date, open_price, take_profit_pct, stop_loss_pct, num_shares):
    ...
    def summary(self):
        msg = "\nOpen price [$]: {0}\n" \
              ...
              "Current return [%]: {6}\n" \
              "Close?: {7}\n" \
              "Current value [$]: {8}".format(self.open_price, self.open_date, self.take_profit_pct, self.stop_loss_price,
                                              self.num_shares, self.current_return, self.return_pct, self.closed, self.current_value)
        print(msg)
    ...
    def update_order(self, current_date, current_price): # 1.2.1 and 1.4.1
        ...
        print("Order current")
        self.current_return = (self.open_price * ((self.return_pct/100) + 1)) * self.num_shares
        # self.current_return = ((current_price - self.open_price) + self.open_price) * self.num_shares
        self.current_value = self.current_return + (self.open_price * self.num_shares)
        # self.current_value = (((self.open_price/100) + 1) + self.open_price) * self.num_shares
        # self.current_value = current_price * self.num_shares
